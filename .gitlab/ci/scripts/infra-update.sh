...
# 1. Load deployment variables from preview stage
if [ -f build.env ]; then
    source build.env
else
    echo "Error: build.env not found. Did the plan stage run successfully?"
    exit 1
fi
...
# 2. Launch Vault Agent to fetch dynamic AWS keys
...
# 3. Source AWS keys and run Terragrunt apply
...
cd "$DEPLOYMENT_PATH"
terragrunt apply -input=false plan.cache
